# Lesson 075: Pre-condition and post-condition loops
...
### 1. IF/ELSE/nested selection, CASE, count-controlled loops, post-condition and pre-condition loops: Pre-condition and post-condition loops (S11.04)
...
#### Worked example: IF/ELSE/nested selection, CASE, count-controlled loops, post-condition and pre-condition loops: Pre-condition and post-condition loops: complete worked route
...
- **pre-condition:** IF/ELSE/nested selection, CASE, count-controlled loops, post-condition and pre-condition…
- **post-condition:** It is well suited when the count or…
- **count-controlled:** A FOR...NEXT loop is count-controlled.
- **loop:** A REPEAT...UNTIL loop is a post-condition loop
- **WHILE:** A WHILE...ENDWHILE loop is a pre-condition loop
- **REPEAT:** FOR when the count is known, WHILE when…
...
3. **Trace state and boundaries** — A REPEAT...UNTIL loop is a post-condition loop

**Concrete case: pre-condition:** IF/ELSE/nested selection, CASE, count-controlled loops, post-condition and pre-condition loops.
...
State two precise facts about pre-condition and post-condition loops.

**Answer:** Use IF statements, including the ELSE clause and nested IF statements; CASE statements; count-controlled loops; post-condition loops; and pre-condition loops. Use IF...THEN...ELSE...ENDIF when a Boolean condition selects between paths. An ELSE clause supplies the false path. In nested IF statements, every inner and outer IF must be closed and the indentation must show which ELSE belongs to which IF.
...
Explain how pre-condition and post-condition loops would be applied in a suitable computing context.

**Answer:** Use IF...THEN...ELSE...ENDIF when a Boolean condition selects between paths. An ELSE clause supplies the false path. In nested IF statements, every inner and outer IF must be closed and the indentation must show which ELSE belongs to which IF. Use CASE...OF...OTHERWISE...ENDCASE when one expression is compared with several discrete values. CASE is not a replacement for range or compound-condition decisions unless the stated values cover the requirement correctly. A count-controlled loop uses FOR...TO...NEXT when the repetition count or inclusive counter range is known before the loop starts. The counter, start value and end value define the iterations; NEXT closes the loop.
...
A student gives an incomplete answer about pre-condition and post-condition loops. Correct it by giving the key distinction and one justified example.
...
- For pre-condition and post-condition loops, use the exact technical term before applying it to the scenario.
