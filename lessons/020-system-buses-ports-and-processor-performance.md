...
1. **Identify the relevant condition or input** — The address bus carries the address of the memory or I/O location being accessed and is normally directed from the processor.
2. **Trace how the process works** — The data bus carries data and instructions in either direction.
3. **Connect the mechanism to its result** — The control bus carries control and timing signals in both directions overall, including read/write signals from the CPU and interrupt or status signals toward it.

#### Worked example: Address, data and control buses: complete worked route

1. **Identify the relevant condition or input**

The address bus carries the address of the memory or I/O location being accessed and is normally directed from the processor.

2. **Trace how the process works**

The data bus carries data and instructions in either direction.

3. **Connect the mechanism to its result**

The control bus carries control and timing signals in both directions overall, including read/write signals from the CPU and interrupt or status signals toward it.

4. **Complete example**

Read memory, then connect a display / Compare two processors for two workloads: To read address 240, the CPU puts 240 on the address bus and read on the control bus; memory returns the contents on the data bus.
...
#### Mastery check (MC-L020-S4.04)

Explain the following targets in one connected answer, using a concrete example for each: address; data; control; buses.
...
- Address bus width If the address bus has n lines, it can represent 2^n different addresses.
- Processor performance depends on processor type, number of cores, bus width, clock speed and cache memory.

**Mechanism or method**

1. **Identify the relevant condition or input** — Processor performance depends on processor type, number of cores, bus width, clock speed and cache memory.
2. **Trace how the process works** — Processor type means the processor architecture and instruction-set design, including how much useful work its execution units can perform for a particular instruction or workload;
3. **Connect the mechanism to its result** — a clock-rate comparison alone is therefore not sufficient.

#### Worked example: Processor performance factors: processor type, cores, bus width, clock and cache: complete worked route
...
Processor type means the processor architecture and instruction-set design, including how much useful work its execution units can perform for a particular instruction or workload;

3. **Connect the mechanism to its result**

a clock-rate comparison alone is therefore not sufficient.
...
#### Mastery check (MC-L020-S4.05)

Explain the following targets in one connected answer, using a concrete example for each: processor type; cores; bus width; clock; cache.
...
- USB is a general serial interface carrying digital data and often power for peripherals. HDMI carries digital video and audio. VGA carries analogue video and does not carry audio in the standard VGA signal. Port choice must match the peripheral and signal rather than rely on a claim that one connector is always best.
- How ports connect peripheral devices, including Universal Serial Bus (USB), High Definition Multimedia Interface (HDMI) and Video Graphics Array (VGA), with accurate signal/use distinctions.
- HDMI carries digital video and audio.
- USB, HDMI and VGA ports.

**Mechanism or method**

1. **Identify the relevant condition or input** — USB is a general serial interface carrying digital data and often power for peripherals.
2. **Trace how the process works** — VGA carries analogue video and does not carry audio in the standard VGA signal.
3. **Connect the mechanism to its result** — Port choice must match the peripheral and signal rather than rely on a claim that one connector is always best.

#### Worked example: USB, HDMI and VGA ports: complete worked route

1. **Identify the relevant condition or input**

USB is a general serial interface carrying digital data and often power for peripherals.

2. **Trace how the process works**

VGA carries analogue video and does not carry audio in the standard VGA signal.

3. **Connect the mechanism to its result**

Port choice must match the peripheral and signal rather than rely on a claim that one connector is always best.

4. **Complete example**

To connect the computer to a modern TV with one digital audio/video cable, choose HDMI. A keyboard or removable drive commonly uses USB, while a legacy analogue display may use VGA.

**Misconceptions to correct**

- Students often memorise register names without roles. Correction: a register earns its name by what it temporarily holds.

#### Mastery check (MC-L020-S4.06)

Explain the following targets in one connected answer, using a concrete example for each: USB; HDMI; VGA; ports.
...
- S4.04: explain address, data, control, buses.
- S4.04 method: Identify the relevant condition or input → Trace how the process works → Connect the mechanism to its result.
- S4.05: explain processor type, cores, bus width, clock, cache.
- S4.05 method: Identify the relevant condition or input → Trace how the process works → Connect the mechanism to its result.
- S4.06: explain USB, HDMI, VGA, ports.
