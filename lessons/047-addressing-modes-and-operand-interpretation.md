...
1. Describe the purpose of **Addressing modes and operand interpretation** using accurate Cambridge AS Computer Science vocabulary.
...
Lesson-specific focus question: What would go wrong if a student confused **Addressing modes and operand interpretation** with a neighbouring syllabus idea?
...
Topic-specific teaching move: keep the explanation anchored to **Addressing modes and operand interpretation**. Students must produce one clear sentence that names the mechanism, one example that uses it, and one exam trap connected to it.
...
## Worked Example
**Problem:** Trace one fetch-decode-execute cycle and name the role of two registers. The worked example must explicitly use **Addressing modes and operand interpretation**, not a generic example from the wider unit.
...
## Student Task
Students annotate a CPU diagram with arrows for one instruction, then explain the path in four precise sentences. Their final answer must include the phrase **Addressing modes and operand interpretation** and one short Chinese support note explaining the hardest word.

## Mini-Quiz
1. State one precise definition connected to **Addressing modes and operand interpretation**.
...
- Answer one 4-mark question about **Addressing modes and operand interpretation** using the structure: point, explanation, context, consequence.
