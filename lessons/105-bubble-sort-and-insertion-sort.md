...
- **B1** linear search initialises Found and Index
- **M1** linear search loops within indexes 1 to 20 until found or exhausted
- **A1** linear search compares Name[Index] with the target and records a match
- **B1** bubble sort uses repeated passes
- **M1** compares adjacent Score[Index] and Score[Index + 1] within valid bounds
- **M1** uses Temp or an equivalent safe three-step swap
- **A1** reduces the unsorted range or uses a valid no-swap stopping condition
- **B1** all constructs close coherently in Cambridge pseudocode

**Strict note:** Do not award only a trace or a description; both requested algorithms must be written and must not access Index + 1 beyond the upper bound.
