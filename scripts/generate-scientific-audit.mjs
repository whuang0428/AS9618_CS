...
const defects = [
  ...
  issue("SCI-CONT-001", "Content completeness", "P0", "High", ["lessons/012-compression-lossless-vs-lossy.md:78", "lessons/013-run-length-encoding-and-dictionary-style-compression.md:78"], "The lessons teach generic lossy/lossless, RLE and dictionary compression but do not teach how bitmap images and vector graphics are compressed; vector compression has no method at all.", "Independent named-term and semantic review of L012-L013; image and sound are mostly used as choice contexts.", "Official Section 1.3 requires how text, bitmap, vector and sound files can be compressed, including RLE.", "A compulsory syllabus method is missing from visible teaching.", "Add distinct worked mechanisms for all four file types, including bitmap RLE and vector object/property compression.", "Require CORE lesson, worked example, practice/MS and direct assessment evidence for all four media types."),
  issue("SCI-CONT-002", "Content correctness", "P0", "High", ["lessons/042-the-fetch-decode-execute-cycle.md:44", "lessons/043-registers-pc-cir-mar-mdr-acc-and-status-register.md:44"], "The fetch-execute flow is described in prose but formal register-transfer notation is not taught or used in the lessons.", "No register-transfer phrase or transfer operator is present in L042-L043 Markdown/HTML; the assessment uses MAR <- PC later.", "Official Section 4.1 requires candidates to describe and use register-transfer notation for the fetch-execute cycle.", "Students are assessed in a notation they were not formally taught.", "Teach explicit transfers such as MAR <- PC and MDR <- Memory[MAR], including brackets and sequencing.", "Require notation in CORE, a traced example, practice and assessment; reject prose-only evidence."),
  issue("SCI-CONT-003", "Content completeness", "P0", "High", ["lessons/046-assembly-language-basics-and-mnemonics.md:84", "lessons/047-addressing-modes-and-operand-interpretation.md:82"], "L046-L047 accurately repair four high-risk instructions but do not teach all official instruction groups or the full specified set; MOV, STO, SUB, INC, DEC, OUT and END are not formally covered.", "Independent comparison against the Version 2 instruction table and group list.", "Official Section 4.2 specifies instruction groups and LDM, LDD, LDI, LDX, LDR, MOV, STO, ADD, SUB, INC, DEC, JMP, CMP, CMI, JPE, JPN, IN, OUT and END.", "Compulsory assembly operations remain absent despite the local coverage gate passing.", "Teach every group and every specified instruction with official effects and operands.", "Assert every opcode/group in visible CORE Markdown and HTML plus direct assessment evidence."),
  issue("SCI-CONT-004", "Content depth", "P1", "High", ["lessons/069-data-validation-and-verification.md"], "Limit check is absent; the official validation method is neither defined, distinguished from range check, nor used.", "Exact-term scan of L069 Markdown and student HTML returned no limit-check teaching.", "Official Section 6.2 explicitly includes range, format, length, presence, existence, limit and check digit.", "Students cannot answer a direct limit-check question from the course.", "Add definition, boundary distinction, example, practice and assessment.", "Require limit check in CORE and reject a list-only mention as sufficient evidence."),
  issue("SCI-CONT-005", "Official terminology", "P1", "High", ["lessons/137-testing-with-normal-boundary-and-erroneous-data.md:1", "web/lesson-137/index.html:56"], "L137 teaches normal, boundary and erroneous data; it never teaches or uses the official category name abnormal.", "Exact-term scan of the responsible lesson and direct comparison with the Version 2 Notes.", "Official Section 12.3 requires normal, abnormal and extreme/boundary test data.", "A locally substituted label can cause students to miss the official command-language category and creates inconsistent terminology with L145.", "Teach abnormal as the official invalid-data category and extreme/boundary as the edge category; retain any synonym only as clarification.", "Require all three official category names in CORE, examples, practice and assessment."),
  issue("SCI-CONT-006", "Content completeness", "P1", "High", ["lessons/049-performance-factors-cores-cache-clock-speed-and-word-length.md:1", "lessons/044-system-buses-address-data-and-control.md:167"], "The performance lessons cover cores, bus width, clock speed and cache, but processor type is absent.", "Whole-project exact-term scan plus semantic review of L044 and L049.", "Official Section 4.1 lists processor type and number of cores, bus width, clock speed and cache memory as performance factors.", "One prescribed performance factor is not taught or assessed directly.", "Add processor type with an architecture/workload boundary and compare it with number of cores.", "Require every official performance factor in CORE and direct assessment evidence."),
  issue("SCI-CROSS-001", "Markdown/HTML parity", "P1", "High", ["lessons/053-operating-system-roles-process-memory-file-and-device-management.md:44", "web/lesson-053/index.html:296"], "Security management is taught in student HTML but is absent from the L052-L053 Markdown source; the Markdown instead mentions antivirus as a utility.", "Cross-carrier search: HTML states user accounts, authentication and access rights; Markdown has no security-management teaching.", "Official Section 5.1 requires security management as an OS task, and the audit requires Markdown and HTML agreement.", "The maintained source and generated student page do not carry the same compulsory evidence.", "Add OS security management to the maintained Markdown/generator source and keep it distinct from antivirus utilities.", "Require the same requirement-specific concepts in Markdown and visible CORE HTML independently."),
  issue("SCI-TERM-001", "Official terminology", "P2", "Medium", ["lessons/010-digital-sound-sampling-rate-sample-resolution-and-duration.md:1", "scripts/stage4-quizzes-data.mjs:30"], "The course consistently uses sample resolution while the Version 2 syllabus term is sampling resolution.", "Terminology scan across L010-L011, HTML and Quiz 10.", "Official Section 1.2 instructs candidates to use sampling, sampling rate and sampling resolution.", "The concept is correctly explained, but terminology differs from the authoritative vocabulary.", "Use sampling resolution as the primary term and mention sample resolution only as a common synonym if retained.", "Official-term checker should require sampling resolution in CORE and assessment wording."),
  ...
  issue("SCI-SEQ-002", "Assessment order", "P1", "High", ["scripts/stage3-assessments-data.mjs:224", "lessons/080-tables-records-fields-data-types-and-constraints.md:42", "lessons/086-sql-joins-and-multi-table-queries.md"], "The L080 monthly assessment requires a normalised three-table design, referential integrity and INNER JOIN before relational design and joins are taught through L081-L086.", "The assessment is scheduled at L080 while its required techniques occur later.", "Assessment must follow direct teaching and practice.", "Students are asked to design and query material not yet taught.", "Move the question to a later assessment or restrict it to concepts taught by L080.", "Map every assessment question to prerequisite lessons and reject later dependencies."),
  issue("SCI-SEQ-003", "Assessment order", "P1", "High", ["scripts/stage4-quizzes-data.mjs:156", "lessons/101-flowcharts-and-pseudocode-notation.md"], "AQ100-Q5 asks for flowchart symbols before L101 formally teaches them.", "Quiz 100 is issued before lesson 101.", "Notation must be taught before recall/application assessment.", "A compulsory notation item is assessed one lesson early.", "Move the item to AQ105 or teach it before Quiz 100.", "Question-to-first-teaching mapping must enforce lesson <= assessment lesson."),
  issue("SCI-SEQ-004", "Curriculum order", "P1", "High", ["lessons/104-linear-search-and-binary-search.md:45", "lessons/105-bubble-sort-and-insertion-sort.md:45", "lessons/115-one-dimensional-arrays.md:134"], "Search and sorting algorithms operate on indexed arrays in L104-L105, but one-dimensional arrays are formally taught in L115.", "The algorithm lessons use list/array indexing before the data structure lesson.", "Data structures and indexing should precede algorithms that traverse and mutate them.", "Learners must infer an undeclared representation while learning the algorithm.", "Teach arrays earlier or explicitly establish the required array model before L104.", "Add semantic prerequisite edge L115 -> L104/L105 and reject backward schedule edges."),
  issue("SCI-SEQ-005", "Assessment order", "P1", "High", ["scripts/stage3-assessments-data.mjs:320", "lessons/115-one-dimensional-arrays.md:134"], "AR112-Q1 requires ARRAY[1:50] OF STRING before L115 teaches array declarations.", "The stage review is at L112; formal array teaching is at L115.", "Stage reviews must not introduce new compulsory syntax.", "A review assesses undeclared syntax.", "Move the item after L115 or teach the declaration earlier.", "Stage-review mappings must reject later prerequisite lessons."),
  issue("SCI-SEQ-006", "Assessment order", "P1", "High", ["scripts/stage3-assessments-data.mjs:237", "lessons/122-stacks-queues-and-linked-lists.md", "lessons/123-choosing-appropriate-data-structures.md"], "The L120 monthly assessment compares stack, queue and linked list before L122-L123 teaches them.", "The assessment lesson number precedes both responsible lessons.", "ADT definitions and operations must precede comparative assessment.", "The assessment requires untaught structures and trade-offs.", "Move the question or move ADT teaching before the L120 checkpoint.", "Direct prerequisite mapping must include all three ADTs."),
  issue("SCI-SEQ-007", "Assessment order", "P1", "High", ["scripts/stage3-assessments-data.mjs:245", "lessons/142-software-development-models.md"], "The L140 monthly assessment compares waterfall, iterative and RAD before L142 teaches the development models.", "The assessment lesson number precedes the responsible CORE lesson.", "Development-model comparison must be taught before evaluation.", "Students face an eight-mark evaluation before formal teaching.", "Move the question after L142 or move the CORE lesson earlier.", "Assessment prerequisite check must fail when teachingLessons are later."),
  issue("SCI-META-001", "Course identity", "P1", "High", ["lessons/137-testing-with-normal-boundary-and-erroneous-data.md:5", "lessons/137-testing-with-normal-boundary-and-erroneous-data.md:68"], "L137 labels testing as Syllabus Section 11 although testing is in official Section 12.3.", "The coverage contract maps the relevant testing requirement to L137 while the page identity says Section 11.", "Student-facing syllabus identity must match the official section.", "The page and generated metadata misclassify a compulsory topic.", "Correct the source metadata and every generated reference.", "Cross-check lesson-declared section against contract ownership and HTML."),
  issue("SCI-VIS-001", "Visual semantics", "P1", "High", ["web/assets/diagrams/stage10-infographics/stage10-lesson-050-shifts.jpg"], "The current eight-bit strings are correctly eight bits, but logical left 10110011 -> 01100110 is labelled 'Overflow: No' even though the discarded high 1 makes unsigned 179 x 2 exceed 255; the adjacent signed arithmetic panel labels the same bit movement as overflow.", "Full-resolution pixel inspection and independent recalculation.", "Fixed-width shift examples must state interpretation and apply overflow consistently.", "The graphic teaches an internally inconsistent boundary rule.", "State unsigned/signed interpretation explicitly and correct or remove the ambiguous overflow label.", "OCR/structured source assertion must recompute each panel's width, result and overflow flag."),
  issue("SCI-VIS-002", "Accessible visual semantics", "P1", "High", ["web/lesson-137/index.html:328", "audits/stage10-ocr-wording.csv:682", "web/assets/diagrams/stage10-infographics/stage10-lesson-137-java.jpg"], "The image visibly contains ELSE and ENDIF, but the screen-reader transcript omits both tokens while claiming to be the infographic text alternative.", "Full-resolution image inspection compared with the student-visible accessibility transcript and OCR ledger.", "Image, transcript, alt text and lesson semantics must agree.", "Screen-reader users receive incomplete pseudocode and a different concept from sighted users.", "Regenerate the transcript from verified semantic source and include every structural token.", "Compare required image tokens with transcript tokens; require ELSE and ENDIF."),
  issue("SCI-VIS-003", "Responsive visual QA", "P2", "High", ["web/stage10-explanations.css:42", "web/stage10-explanations.css:50"], "At <=640 px, Stage 10 images are forced to 720 px with container overflow-x:auto; L050's shift image overrides this but becomes too small to read at 390 px.", "Rendered 390x844 scan plus CSS inspection; document overflow is zero but the infographic containers intentionally overflow.", "The stated acceptance requires no page or container horizontal overflow and readable text.", "Most infographics require horizontal panning; the exception sacrifices legibility.", "Adopt responsive image-specific layouts or responsive alternate assets instead of a global 720 px rule.", "At 390x844 assert container scrollWidth <= clientWidth and minimum readable text size."),
  issue("SCI-VIS-004", "Visual audit gate", "P1", "High", ["audits/stage10-semantic-review-register.csv:176", "audits/stage10-imagegen-assets.md:181"], "The existing register marks L050 shifts Approved with two reviews and no defect, yet independent inspection finds SCI-VIS-001.", "Current asset hash matches the Approved register row.", "Existing Approved status and matching hashes are evidence of identity, not semantic correctness.", "The two-pass visual gate has a demonstrated false negative.", "Reopen all visual rows for independent review and add deterministic calculation assertions.", "Mutation test the overflow flag and require the visual verifier to fail."),
  issue("SCI-WEB-001", "Mobile usability", "P2", "Medium", ["web/lesson-toolbar.css:24", "web/lesson-toolbar.css:140"], "The fixed two-row lesson toolbar occupies and overlays the lower part of the 390x844 viewport while reading lesson content.", "Rendered mobile screenshot of L050; the page adds bottom padding but the control still obscures content during scrolling.", "Controls must not overlap teaching content at the specified mobile viewport.", "Visible reading area is reduced and text can sit behind the toolbar until further scrolling.", "Collapse the toolbar or reserve non-overlapping layout space on narrow screens.", "Screenshot assertion at top, middle and end of representative long lessons."),
  issue("SCI-TEACH-001", "Teaching usability", "P2", "Medium", ["lessons/027-computer-system-overview-input-output-storage-processor-and-memory.md", "lessons/039-environmental-and-reliability-considerations-in-hardware.md"], "After generated repair/explanation blocks and topic labels are normalised away, seven L027-L039 lessons share an identical base scaffold; L008/L009 and L052/L058 form additional exact pairs.", "Deterministic Jaccard/template normalisation found exact equality in these groups; this is a review signal, not proof that all generated inserts are identical.", "Examples and practice should be specific enough to teach and transfer each lesson's concept.", "Generic prompts can satisfy structural checks while providing shallow or repetitive learning.", "Manually review the flagged groups and replace only genuinely generic examples/tasks with topic-specific work.", "Add a similarity gate as a warning and require human disposition for exact normalised matches."),
  issue("SCI-ASSESS-001", "Mark scheme coding", "P2", "Medium", ["scripts/ms-review-utils.mjs:110"], "Twenty-two questions contain A1 marks without any M mark in the same question; several use A1 as a generic exact-answer mark after B points.", "Independent scan of all 963 questions; examples include L003-Q2, L087-Q4, L102-Q1 and AQ010-Q1.", "B/M/A codes should reflect independent knowledge, method and dependent accuracy consistently.", "Inconsistent code semantics weakens marking reliability and follow-through behaviour.", "Review the 22 flagged questions against the official mark-scheme convention and recode where necessary.", "Require an explicit dependency rationale for every A mark without M."),
  issue("SCI-ASSESS-002", "Mark scheme coding", "P2", "Medium", ["web/lesson-035/app.js", "web/lesson-036/app.js", "web/lesson-040/app.js"], "Five logic questions offer follow-through guidance although every mark is coded B1; the dependency boundary is not expressed in the mark codes.", "963-question scan found L035-Q3, L036-Q1, L036-Q2, L036-Q4 and L040-Q3.", "Follow-through should attach to a clear preceding calculated result/method.", "Markers may apply FT inconsistently.", "Define method/accuracy dependencies or remove FT where answers are independently marked.", "Validate every FT note against at least one explicit M/A dependency."),
  issue("SCI-ASSESS-003", "Assessment metadata", "P2", "High", ["web/lesson-001/app.js:92", "scripts/ms-review-utils.mjs:55"], "The 750 lesson exam questions contain no explicit AO metadata; only the 213 bank questions inherit assessment-level AO lists.", "Question inventory count is 963 = 750 lesson + 213 bank; lesson question objects expose prompt, answer, marks, marking and strict fields only.", "The requested requirement x assessment x AO matrix needs auditable AO evidence.", "AO balance for most questions cannot be independently verified without inference.", "Add reviewed question-level AO metadata or a separate stable mapping.", "Reject Unknown AO values before release approval."),
];
...
const baseline = {
  ...
  official,
  scope: { lessons: 150, pages: 153, pageViews: 306, questions: 963, assessmentSets: 51, stage10Images: 783, visualObjects: 969, syllabusRequirements: 121 },
  mutationTests: [
    ...
    { id: "MUT-COV-002", mutation: "Changed every L050 CORE region to OPTIONAL in the same temporary mirror.", expected: "Fail S4.15", actual: "Verifier failed S4.15 CORE sections", result: "DetectionConfirmedForOverride" },
  ],
  pixelOcr: {
    method: "macOS Vision fast recognition over the actual 783 JPEG assets; run outside the restricted sandbox after the in-sandbox CVPixelBuffer failure was diagnosed.",
...
const questionDefects = new Map([
  ["L001-Q1", "SCI-SEQ-001"], ["AM080-Q4", "SCI-SEQ-002"], ["AQ100-Q5", "SCI-SEQ-003"], ["AR112-Q1", "SCI-SEQ-005"], ["AM120-Q3", "SCI-SEQ-006"], ["AM140-Q4", "SCI-SEQ-007"],
]);
const bOnlyFt = new Set(["L035-Q3", "L036-Q1", "L036-Q2", "L036-Q4", "L040-Q3"]);
...
const routes = ["index", "resources", "assessments", ...Array.from({ length: 150 }, (_, index) => `lesson-${String(index + 1).padStart(3, "0")}`)];
const browserRows = [];
for (const route of routes) {
  for (const viewport of ["1440x900", "390x844"]) {
    ...
    const interaction = route === "assessments" ? "Pass:filters-MS-reset-empty" : route === "lesson-050" ? "Pass:contents-answer" : "PendingByComponentType";
    browserRows.push([route, viewport, "Pass", "Pass", "Pass", containerOverflow, "PassFreshPort", interaction,
      ...
      route === "lesson-100" ? "Fresh-port cache-busted retest produced zero console errors." : "Rendered through local HTTP server."]);
  }
}
...
const graph = {
  ...
  edges: [
    ...
    ["L012", "L013", "compression categories before algorithms"], ["L041", "L042", "CPU components before fetch-execute"], ["L042", "L043", "cycle before register detail"],
    ["L043", "L045", "registers before instruction format"], ["L045", "L046", "instruction format before assembly"], ["L046", "L047", "syntax before addressing trace"],
    ["L078", "L079", "database concepts before relational comparison"], ["L079", "L080", "relational model before tables and keys"], ["L080", "L081", "fields and keys before key design"],
    ["L081", "L082", "keys before ER modelling"], ["L082", "L083", "relationships before normalisation"], ["L083", "L084", "schema before SQL"], ["L084", "L085", "single-table SELECT before aggregate SQL"],
    ["L085", "L086", "single-table queries before joins"], ["L098", "L099", "problem solving before abstraction"], ["L099", "L100", "abstract model before constructs"],
    ["L100", "L101", "constructs before notation"], ["L101", "L102", "notation before tracing"], ["L102", "L103", "trace before defensive validation"],
    ["L115", "L104", "array representation before search"], ["L115", "L105", "array representation before sort"], ["L113", "L115", "data types before arrays"],
    ["L115", "L116", "one-dimensional before two-dimensional arrays"], ["L115", "L117", "array bounds before counting"], ["L113", "L118", "types before records"],
    ["L118", "L119", "records before arrays of records"], ["L115", "L122", "array storage before array-based ADTs"], ["L122", "L123", "ADT definitions before selection"],
    ["L130", "L131", "subprogram interface before parameter passing"], ["L131", "L132", "parameters before scope"], ["L142", "AM140", "development models before monthly assessment"],
