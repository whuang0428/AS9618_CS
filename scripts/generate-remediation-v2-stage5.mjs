...
const closures = {
  ...
  "RV2-CONT-001": [
    "L049 topic heading, teaching, assessment, maintained transcript and deterministic concept image use processor type and number of cores, bus width, clock speed and cache memory",
  ...
  "RV2-PSEUDO-001": [
    "L107/L133 keep MID results as STRING and reserve LCASE/UCASE for declared CHAR values with single-quoted literals",
    "LEFT is identified as guide-external and is not called without a complete question-supplied signature",
  ],
  "RV2-PSEUDO-002": [
    "L121 defines SPLIT, STRING_TO_INTEGER, IS_NUMERIC and FIELD_COUNT with complete typed signatures before use",
...
const gateResult = {
  ...
  unverified: ["Stage 6 full 783-image two-pass census", "Stage 6 969-visual-object and 306-browser-record QA", "Stage 7 independent final/reproducibility gate"],
};
...
- Aligned L049 performance-factor teaching, assessment, transcript and deterministic image with the complete official list: processor type and number of cores, bus width, clock speed and cache memory.
- Enforced the pseudocode-guide CHAR/STRING boundary in L107/L133 and supplied complete typed signatures/index rules for guide-external functions in L121.
...
- Technical gate: Ready; zero L049/L107/L121/L133 failures.
...
- Browser: four desktop pages and four 390px records have zero horizontal overflow; eight critical images load; answer details expand; L133 rejects STRING input to UCASE and accepts one CHAR; console warnings/errors=0.
...
- The full 783-image structural verifier passes, but its old Approved rows are not treated as the Stage 6 independent census.
...
- L049 word length remains available only inside a visibly labelled Optional enrichment section and is excluded from the official performance-factor coverage and assessment contract.
...
- Stage 6 first-pass/reverse-pass review of all 783 Stage 10 images.
- Stage 6 969 visual-object checks and 153-page desktop plus 390px browser matrix (306 records).
- Stage 7 independent high-risk review and double-generation release gate.

## Remaining risks

- The eight Stage 5 images are closed, but this does not transfer approval to the other 775 images.
