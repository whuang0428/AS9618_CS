...
function assessmentHtml(entry, type, index) {
  const questions = entry.questions.map((question, qIndex) => `<article class="question"><p><strong>Q${qIndex + 1}.</strong> ${escapeHtml(question.prompt)} <span>[${question.marks}]</span></p><details><summary>Show mark scheme</summary><ul>${question.points.map(([code, text]) => `<li><strong>${code}</strong> ${escapeHtml(text)}</li>`).join("")}</ul></details></article>`).join("");
...
function renderWeb() {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>AS9618 Assessment Bank</title><link rel="icon" href="data:"><link rel="stylesheet" href="./styles.css"><link rel="stylesheet" href="../stage7-accessibility.css?v=3"></head>
...
<section class="assessment-list" id="assessmentList">${cards}</section></main><script src="./app.js"></script><script src="../stage7-accessibility.js?v=4"></script></body></html>`;
}
