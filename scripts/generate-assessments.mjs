...
function questionMarkdown(question, index) {
  return `${index + 1}. ${normaliseQuestionPrompt(question.prompt)} **[${question.marks}]**`;
}
...
|---|---|---:|
${question.points.map(([, text], pointIndex) => `| ${text.replaceAll("|", "\\|")} | ${pointIndex === 0 ? question.note.replaceAll("|", "\\|") : ""} | 1 |`).join("\n")}`;
}
...
function assessmentHtml(entry, type, index) {
  const questions = entry.questions.map((question, qIndex) => {
    const prompt = normaliseQuestionPrompt(question.prompt);
    const rows = question.points.map(([, text], pointIndex) => `<tr><td>${escapeHtml(text)}</td><td>${pointIndex === 0 ? escapeHtml(question.note) : ""}</td><td>1</td></tr>`).join("");
    return `<article class="question" data-marks="${question.marks}" data-command="${commandWord(prompt)}"><p><strong>Q${qIndex + 1}.</strong> ${escapeHtml(prompt)} <span>[${question.marks}]</span></p><details><summary>Show mark scheme</summary><table class="mark-scheme-table"><thead><tr><th>Answer</th><th>Guidance</th><th>Marks</th></tr></thead><tbody>${rows}</tbody></table></details></article>`;
  }).join("");
...
function renderWeb() {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>AS9618 Assessment Bank</title><link rel="icon" href="data:"><link rel="stylesheet" href="./styles.css"><link rel="stylesheet" href="../stage7-accessibility.css?v=3"><link rel="stylesheet" href="../academic-theme.css?v=7"></head>
...
const markSchemeStyles = `.mark-scheme-table{width:100%;border-collapse:collapse}.mark-scheme-table :where(th,td){border:1px solid var(--line);padding:8px;text-align:left;vertical-align:top}.mark-scheme-table :where(th,td):last-child{width:5rem;text-align:center}`;
