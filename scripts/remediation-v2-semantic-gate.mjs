...
export function actualCriticalFixture() {
  ...
  const l050 = markdownLesson(50);
  const l108 = markdownLesson(108);
  const l122 = markdownLesson(122);
  const l134 = markdownLesson(134);
  const l050Html = webLessonFile(50, "index.html");
  const l050App = webLessonFile(50, "app.js");
  const l108Html = webLessonFile(108, "index.html");
  const l108App = webLessonFile(108, "app.js");
  const l122Html = webLessonFile(122, "index.html");
  const l122App = webLessonFile(122, "app.js");
  const l134Html = webLessonFile(134, "index.html");
  const l134App = webLessonFile(134, "app.js");
  return {
    onesComplement: {
    ...
    performance: {
      title: `${l050.match(/^## Optional enrichment:.*$/m)?.[0] ?? ""} ${l050Html.match(/<section[^>]+id="concept"[\s\S]*?<h2>([\s\S]*?)<\/h2>/i)?.[1] ?? ""}`,
      teaching: `${l050} ${l050Html}`,
      practice: l050App,
      assessment: questionsForLesson(50),
      visual: `${visualFacts(50, "explanation-concept")} ${ocrFacts("stage10-lesson-050-concept.jpg")}`,
    },
    l107: `${l108} ${l108Html} ${l108App} ${visualFacts(108, "explanation-pseudocode")} ${ocrFacts("stage10-lesson-108-pseudocode.jpg")}`,
    l121: `${l122} ${l122Html} ${l122App} ${visualFacts(122, "explanation-parse")} ${visualFacts(122, "explanation-pseudocode")} ${visualFacts(122, "explanation-types")} ${ocrFacts("stage10-lesson-122-parse.jpg")} ${ocrFacts("stage10-lesson-122-pseudocode.jpg")} ${ocrFacts("stage10-lesson-122-types.jpg")}`,
    l133: `${l134} ${l134Html} ${l134App} ${visualFacts(134, "explanation-case")} ${visualFacts(134, "explanation-concat")} ${visualFacts(134, "explanation-java")} ${visualFacts(134, "explanation-position")} ${ocrFacts("stage10-lesson-134-case.jpg")} ${ocrFacts("stage10-lesson-134-concat.jpg")} ${ocrFacts("stage10-lesson-134-java.jpg")}`,
  };
}
...
export function evaluateCriticalSemanticControls(fixture = actualCriticalFixture()) {
  ...
  for (const term of ["processor type", "number of cores", "bus width", "clock speed", "cache"]) {
    pushMissingSurface(problems, "CRIT-L050-PERFORMANCE-FACTORS", "official processor performance list", performanceSurfaces, term);
  }
  for (const surface of ["title", "visual"]) {
    if (includes(performanceSurfaces[surface], "word length")) {
      problems.push({ id: "CRIT-L050-PERFORMANCE-FACTORS", type: "CRITICAL_SEMANTIC_CONTROL", location: surface, detail: `${surface} substitutes word length into the official performance-factor list` });
    }
  }

  if (/\b(?:LCASE|UCASE)\s*\(\s*MID\s*\(/i.test(fixture.l107)) {
    problems.push({ id: "CRIT-L108-CHAR-FUNCTION-TYPE", type: "CRITICAL_SEMANTIC_CONTROL", location: "L108", detail: "MID returns STRING and is passed directly to CHAR-only LCASE/UCASE" });
  }
  ...
  if (!suppliedSignature(fixture.l121, "SPLIT", splitSignature, oneBasedConvention)) {
    problems.push({ id: "CRIT-L122-PROVIDED-FUNCTIONS", type: "CRITICAL_SEMANTIC_CONTROL", location: "L122", detail: "SPLIT is used without a complete supplied signature and indexing convention" });
  }
  if (!suppliedSignature(fixture.l121, "STRING_TO_INTEGER", stringToIntegerSignature)) {
    problems.push({ id: "CRIT-L122-PROVIDED-FUNCTIONS", type: "CRITICAL_SEMANTIC_CONTROL", location: "L122", detail: "STRING_TO_INTEGER is used without a complete supplied signature and indexing convention" });
  }
  ...
  if (!suppliedSignature(fixture.l121, "IS_NUMERIC", isNumericSignature)) problems.push({ id: "CRIT-L122-PROVIDED-FUNCTIONS", type: "CRITICAL_SEMANTIC_CONTROL", location: "L122", detail: "IS_NUMERIC is used without a complete supplied signature" });
  if (!suppliedSignature(fixture.l121, "FIELD_COUNT", fieldCountSignature)) problems.push({ id: "CRIT-L122-PROVIDED-FUNCTIONS", type: "CRITICAL_SEMANTIC_CONTROL", location: "L122", detail: "FIELD_COUNT is used without a complete supplied signature" });

  if (/\b(?:LCASE|UCASE)\s*\(\s*(?:MID\s*\(|Text\b|Name\b|FirstName\b|Word\b)/i.test(fixture.l133)) {
    problems.push({ id: "CRIT-L134-CHAR-FUNCTION-TYPE", type: "CRITICAL_SEMANTIC_CONTROL", location: "L134", detail: "CHAR-only LCASE/UCASE is applied to a STRING expression" });
  }
  if (/\b(?:LCASE|UCASE)\s*\(\s*Answer\s*\)/i.test(fixture.l133) && !/DECLARE\s+Answer\s*:\s*CHAR/i.test(fixture.l133)) {
    problems.push({ id: "CRIT-L134-CHAR-FUNCTION-TYPE", type: "CRITICAL_SEMANTIC_CONTROL", location: "L134", detail: "Answer is passed to LCASE/UCASE without an explicit CHAR declaration" });
  }
  ...
  if (!suppliedSignature(fixture.l133, "LEFT", leftSignature, oneBasedConvention)) {
    problems.push({ id: "CRIT-L134-PROVIDED-FUNCTION", type: "CRITICAL_SEMANTIC_CONTROL", location: "L134", detail: "LEFT is used without a complete question-provided signature and position convention" });
  }
  if (/\b(?:LCASE|UCASE)\s*\(\s*"/i.test(fixture.l133)) problems.push({ id: "CRIT-L134-CHAR-LITERAL", type: "CRITICAL_SEMANTIC_CONTROL", location: "L134", detail: "CHAR-only LCASE/UCASE receives a double-quoted STRING literal" });
  ...
  for (const [lesson, text] of [[108, fixture.l107], [122, fixture.l121], [134, fixture.l133]]) {
