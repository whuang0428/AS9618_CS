...
const lessonHtml = Array.from({ length: 151 }, (_, index) => fs.readFileSync(path.join(root, "web", `lesson-${String(index + 1).padStart(3, "0")}`, "index.html"), "utf8"));
...
if (questions.length !== 968 || review.entries.length !== 968) fail(`Expected 968 current questions and reviews; found ${questions.length}/${review.entries.length}`);
...
console.log(`CIE wording verification passed: 27 official command words, 968 item-level reviews with four trials each, ${evaluation.counts.approved} current-hash approvals and no student-visible B1/M1/A1 codes.`);
