...
const examQuestions = [
  {
    ...
    marking: [
      ...
      { mark: "M1", text: "encryption uses key/algorithm and can be decrypted with correct key" },
      { mark: "B1", text: "hashing creates a digest/hash value from input data" },
      { mark: "M1", text: "hashing is one-way/not intended to recover original data" },
      { mark: "A1", text: "valid use comparison, e.g. encryption for confidentiality; hashing for password verification/integrity" },
    ],
    strict: [
      ...
      "FT: award use mark if mechanisms are partly correct.",
    ],
  },
  {
    title: "Question 2",
    ...
    marking: [
      ...
      { mark: "M1", text: "hash/digest rather than plaintext password is stored" },
      { mark: "B1", text: "entered password is hashed at login" },
      { mark: "M1", text: "new hash compared with stored hash" },
      { mark: "A1", text: "security benefit linked to not storing plaintext/reduced exposure" },
    ],
    strict: [
      ...
      "FT: award comparison mark if hash generation is correctly described.",
    ],
  },
  {
    title: "Question 3",
    ...
    marking: [
      ...
      { mark: "M1", text: "valid certificate content, e.g. public key/domain/owner/issuer/expiry/signature" },
      { mark: "B1", text: "browser checks trusted issuer/certificate authority" },
      { mark: "M1", text: "browser checks validity such as domain match/expiry/signature" },
      { mark: "A1", text: "consequence: authenticates site and/or supports encrypted HTTPS communication" },
    ],
    strict: [
      ...
      "FT: award consequence if identity-public key link is clear.",
    ],
  },
  {
    title: "Question 4",
    ...
    answer: "Stored passwords should be hashed because the shop only needs to verify an entered password by comparing hashes and should not need to recover the plaintext password. Payment data sent over the internet should be encrypted because the data must remain confidential while in transit and must be recoverable by the intended recipient. A digital certificate should also be used for the website so the browser can check the website identity and public key before setting up secure communication.",
    marking: [
      { mark: "B1", text: "stored passwords use hashing" },
      { mark: "M1", text: "justification linked to one-way comparison/no plaintext recovery needed" },
      { mark: "B1", text: "payment data in transit uses encryption" },
      { mark: "M1", text: "justification linked to confidentiality and intended recipient can decrypt/read" },
      { mark: "B1", text: "digital certificate used for website/public key identity" },
      { mark: "A1", text: "certificate justification linked to browser trust/HTTPS/CA checks" },
    ],
    strict: [
      ...
      "FT: award each method independently.",
    ],
  },
  {
    title: "Question 5",
    ...
    marking: [
      ...
      { mark: "M1", text: "calculated hash compared with published/expected hash" },
      { mark: "B1", text: "matching hashes indicate file likely unchanged" },
      { mark: "M1", text: "different hashes indicate changed/corrupted/tampered file" },
      { mark: "A1", text: "reason linked to changed input producing different hash value" },
    ],
    strict: [
      ...
      "FT: award tampering mark if expected vs calculated comparison is implied.",
    ],
  },
];
